...
function getLogoHTML(val, type, isLarge = false) {
    ...
    if (fileMap[key]) {
        ...
        // Le SVG est généré. S'il ne charge pas, on affiche un texte de fallback en secours.
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function showQuestion() {
        ...
        // 1. Le SVG apparait en grand titre
        ...
        if (q.isMultiple) {
            ...
            q.options.forEach(opt => {
                ...
                // 3. Afficher UNIQUEMENT le SVG sur lequel on doit cliquer pour les lignes
                if (q.optType === 'metro') {
                    label.classList.add('logo-only-label'); // Cache la case à cocher, centre le logo
                    span.innerHTML = `<div class="logo-container">${getLogoHTML(opt, 'M')}</div>`;
                } else if (q.optType === 'rer_tram') {
                    let logo = parseRerTramLogo(opt);
                    if(logo !== "Aucunes") {
                        label.classList.add('logo-only-label');
                        span.innerHTML = `<div class="logo-container">${logo}</div>`;
                    } else {
    ...
    function formatAnswersForRecap(arr, optType) {
        if (arr.length === 1 && arr[0] === "Aucune réponse") {
            return '<span style="font-style:italic;">Aucune sélectionnée</span>';
        }
        // 2. Transformer les textes de réponse en logos pour le récap final
        if (optType === 'metro') {
            return `<div class="recap-logo-list">${arr.map(val => getLogoHTML(val, 'M')).join('')}</div>`;
        } else if (optType === 'rer_tram') {
            return `<div class="recap-logo-list">${arr.map(val => parseRerTramLogo(val)).join('')}</div>`;
        } else {
            return arr.join(', ');
        }
    }
    ...
    function endGame() {
        ...
        gameHistory.forEach((item, index) => {
            ...
            let userStr = formatAnswersForRecap(item.user, item.optType);
            let corrStr = formatAnswersForRecap(item.correct, item.optType);
