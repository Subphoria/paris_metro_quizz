    ...
    function showQuestion() {
        ...
        if (q.isMultiple) {
            ...
            q.options.forEach(opt => {
                ...
                // Fix bug: use a span instead of innerHTML on the label directly
                const span = document.createElement('span');
                if (q.optType === 'metro') {
                    span.innerHTML = `<div class="logo-container">${getLogoHTML(opt, 'M')} Ligne ${opt}</div>`;
                } else if (q.optType === 'rer_tram') {
                    span.innerHTML = `<div class="logo-container">${parseRerTramLogo(opt)} ${opt !== 'aucunes' ? '' : ''}</div>`;
                } else {
